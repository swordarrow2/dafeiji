...
public class JudgingSystem {
    ...
    private static AtomicReference<Entry<ImmutableWrapper<Ellipse>, PlayerCollisionData>> mJudgeEntry = new AtomicReference<>(null);
    ...
    public static PlayerCollisionData playerCollision() {
        mJudgeEntry.set(null);
        for (Entry<ImmutableWrapper<Ellipse>, PlayerCollisionData> entry : enemyJudges.entrySet()) {
            ImmutableWrapper<Ellipse> key = entry.getKey();
            PlayerCollisionData playerCollisionData = entry.getValue();
            if (playerCollisionData.judgeCallback.getDamage() <= 0 || Math.max(key.getData().width, key.getData().height) < 0.1f) {
                continue;
            }
            Vector2 vector2 = new Vector2(playerJudge);
            vector2.sub(key.getData().x, key.getData().y).rotate(-playerCollisionData.transform.rotation).add(key.getData().x, key.getData().y);
            if (key.getData().contains(vector2)) {
                mJudgeEntry = (AtomicReference<Entry<ImmutableWrapper<Ellipse>, PlayerCollisionData>>) entry;
            }
        }
        return mJudgeEntry == null ? null : mJudgeEntry.get().getValue();
		/*
		...
		stream.forEach((entry) -> {
			...
			if (entry.getKey().getData().contains(tmp)) {
				mJudgeEntry.getAndSet(entry);
			}
		});
		stream.close();
		return mJudgeEntry.get() == null ? null : mJudgeEntry.get().getValue();
		*/
    }
